...
parallel_speedup_graph = function (df)
{
  ...
  names(with_total_time) = c("Task", "Variant", "Threads", "Time")
  ...
  ggsave('opt_parallel_speedup.pdf', p, height=10, width=12, units="cm", dpi=600)
}


splits = split_comm_time(results)

splits = summaryBy(TotalTime + Time ~ TimeType + Variant + Task + Threads,
  data = splits,
  FUN = list (mean, sd))

## print (splits[splits$Variant == 'All',])
